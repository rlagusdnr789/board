...
<%
	//boardList.jsp페이지는 currentPage라는 매개변수가 넘어와야 하는데
	// 매개변수가 안 넘어노는 경우 currentPage를 1이 대입
	int currentPage = 1;
...
%>
	<table class="table table-striped">
		...
		<tbody>
<%
		int pagePerRow= 10; //페이지당 보여줄 글의 목록을 10개로 설젖ㅇ
...
<%
	//마지막 페이지는 전체글의 수를 pagePerRow로 나누었을때 나누어 떨어지면 몫이 마지막 페이지
	//ex)전체글이 50개/10개씩 ->마지막 페이지는 5페이지
	//나누어 떨어지지 않으면
	//ex) 전체글ㄹ이 51개/ 10개씩 -> 마지막 페이지는 6페이지
...
<%
	...
	if(currentPage<lastPage){ //현재페이지가 마지막 페이보다 작으면 다음페이지보다 작으면 다음페이지 링크를 추가
%>	
		<li class="previous"><a href="<%=request.getContextPath()%>/model1_board/boardList.jsp?currentPage=<%=currentPage+1%>">다음</a></li>
<%
	}
%>	
	</ul>
